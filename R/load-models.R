...
.forestgrowthmatrixP2_models <- new.env()
...
.load_models <- function() {
  ...
  cache_dir <- getOption("forestgrowthmatrixP2.cache_dir")
  ...
  for (model_type in names(MODEL_URLS)) {
    ...
    if (exists(model_type, envir = .forestgrowthmatrixP2_models)) {
    ...
    if (success) {
      tryCatch({
        .forestgrowthmatrixP2_models[[model_type]] <- readRDS(cache_path)
...
models_loaded <- function() {
  ...
  all(required_models %in% ls(envir = .forestgrowthmatrixP2_models))
}
...
get_model <- function(model_name = c("mortality", "upgrowth", "recruitment")) {
  ...
  if (!exists(model_name, envir = .forestgrowthmatrixP2_models)) {
    stop("Model '", model_name, "' not found in loaded models.")
  }

  return(get(model_name, envir = .forestgrowthmatrixP2_models))
}
...
clear_model_cache <- function() {
  ...
  rm(list = ls(envir = .forestgrowthmatrixP2_models), envir = .forestgrowthmatrixP2_models)
}
